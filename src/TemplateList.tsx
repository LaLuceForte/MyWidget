...
import { ISetCursor, type IfThenElse, type TemplateObject } from './MessageTemplateEditor'
...
function TemplateList (props: TemplateListProps): JSX.Element {
  ...
  const handleAddTextarea = (bluredIndex: number, buttonName: string, template: string, id: string | undefined, position: number|undefined, textareaId: string|undefined): void => {
    ...
    if (id === undefined) {
      if (props.innerId!==''){
        params = { bluredIndex: bluredIndex, position: position!, textareaId: props.innerId!, id: id}
        props.handleSetParams({ bluredIndex: bluredIndex, position: position!, textareaId: props.innerId!, id: id})
      }
        ...
        if (bluredIndex > 0) {
          props.handleBlured(Object.assign(
            [...list.slice(0, bluredIndex),
               { ...list[bluredIndex], template: list[bluredIndex].textareaRef.current?.value.substring(0, position), 
                ifThenElse: { if: [{ textareaRef: React.createRef(), template: '' }], 
                then: [{ textareaRef: React.createRef(), template: '' }], else: [{ textareaRef: React.createRef(), template: '' }] } }, 
                { ...list[bluredIndex], template: list[bluredIndex].textareaRef?.current?.value.substring(position!), 
                  textareaRef: React.createRef(), ifThenElse: list[bluredIndex].ifThenElse }, ...list.slice(bluredIndex + 1)]),
          ...
          props.handleBlured(Object.assign(
            [{ ...list[0], template: list[0].textareaRef?.current?.value.substring(0, position),
               ifThenElse: { if: [{ textareaRef: React.createRef(), template: '' }],
                then: [{ textareaRef: React.createRef(), template: '' }], 
                else: [{ textareaRef: React.createRef(), template: '' }] } }, 
                ...list.slice(1, bluredIndex + 1), 
                { ...list[0], template: list[0].textareaRef?.current?.value.substring(position!), textareaRef: React.createRef(), 
                  ifThenElse: list[0].ifThenElse }, ...list.slice(bluredIndex + 1)]), textareaId, buttonName)
      ...
      props.handleBlured(Object.assign(
        [...list.slice(0, bluredIndex), { ...list[bluredIndex], template:template.substring(0,position)+'{'+id+'}'+template.substring(position!, template.length), 
        textareaRef: list[bluredIndex].textareaRef }, ...list.slice(bluredIndex + 1)]),undefined,buttonName,id,position, textareaId)
  ...
  const setId = (obj: TemplateObject) => {
    setObj(obj)
  }

  const setId2 = (obj2: ISetCursor) => {
    setObj2(obj2)
  }
        ...
        props.handleBlured(Object.assign(
          [{ ...list[0], template: '',
             ifThenElse: { if: [{ textareaRef: React.createRef(), template: '' }],
              ...
              ...list.slice(1, 1), 
              { ...list[0], template: list[0].textareaRef?.current?.value, textareaRef: React.createRef(), 
                ifThenElse: list[0].ifThenElse }, ...list.slice( 1)]))

        setList(Object.assign([{ ...list[0], template: '',
         ifThenElse: { if: [{ textareaRef: React.createRef(), template: '' }], 
         then: [{ textareaRef: React.createRef(), template: '' }], else: [{ textareaRef: React.createRef(), template: '' }] } },
          ...list.slice(1, 1), { ...list[0], template: list[0].textareaRef?.current?.value, 
            textareaRef: React.createRef(), ifThenElse: list[0].ifThenElse }, ...list.slice(1)]))
  ...
  const handleVarClicked = () => {
    ...
    setTimeout(()=>{
       ...
       if (params.textareaId === ''){
           props.handleBlured(Object.assign(
        [...list.slice(0, params.bluredIndex), { ...list[params.bluredIndex], template:list[params.bluredIndex].textareaRef.current?.value.substring(0,params.position)+'{'+props.varClicked?.id+'}'+list[params.bluredIndex].textareaRef.current?.value.substring(params.position!, list[params.bluredIndex].textareaRef.current?.value.length), 
        textareaRef: list[params.bluredIndex].textareaRef }, ...list.slice(params.bluredIndex + 1)]))
      setList(Object.assign(
        [...list.slice(0, params.bluredIndex), { ...list[params.bluredIndex],template:list[params.bluredIndex].textareaRef.current?.value.substring(0,params.position)+'{'+props.varClicked?.id+'}'+list[params.bluredIndex].textareaRef.current?.value.substring(params.position!, list[params.bluredIndex].textareaRef.current?.value.length), 
        textareaRef: list[params.bluredIndex].textareaRef }, ...list.slice(params.bluredIndex + 1)]))
      ...
      // добавляем блок переменную в место, где указывался курсор в последний раз
      if (props.innerId===params.textareaId) {
        props.handleBlured(Object.assign(
          [...list.slice(0, params.bluredIndex), { ...list[params.bluredIndex], template:list[params.bluredIndex].textareaRef.current?.value.substring(0,params.position)+'{'+props.varClicked?.id+'}'+list[params.bluredIndex].textareaRef.current?.value.substring(params.position!, list[params.bluredIndex].textareaRef.current?.value.length), 
          textareaRef: list[params.bluredIndex].textareaRef }, ...list.slice(params.bluredIndex + 1)]))
